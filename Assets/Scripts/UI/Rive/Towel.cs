...
{
    /// <summary>
    /// Binds the dedicated towel artboard widget. DryHandsStage enables Unity
    ...
    public sealed class Towel : MonoBehaviour, ITowelControl
    {
        ...
        public const string Hitbox = "towel-hitbox";

        public const string IsDragged = "isDragged";

        public const string AnimTimeline1 = "Timeline 1";
        ...
        public void Bind(RiveWidget towelWidget, Hands handsTarget = null)
        {
            widget = towelWidget;
            if (handsTarget != null)
                hands = handsTarget;
        ...
        /// <summary>
        /// Towel's SM uses isDragged for an authored lift pose. Unity already
        /// moves the panel, so keep the input false to avoid clipping.
        /// </summary>
        void PinIsDraggedFalse()
        {
            ...
            {
                _loggedInputs = true;
                Debug.Log($"[Towel] SM inputs: {RiveStateMachineInputs.DescribeInputs(widget.StateMachine)}");
            }
